...
/* TODO: impliment more generic way for datasetdir */
char *datasetdir = "datasets";
...
bool configurationExists(void)
{
	...
	fclose(infile);
	snprintf(datasetPath, MAX_PATH_LENGTH, "%s/../../%s", cwd, datasetdir);
	if ((indir = opendir(datasetPath)) == NULL)
	{
		fprintf(stderr, "Dataset directory not fount. Directory %s not found...", datasetPath);
		return false;
	}
	closedir(indir);
	return true;
...
void writeConfig()
{
	...
	{
		fprintf(stderr, "File %s not found...", conffile);
		return false;
	}

	snprintf(datasetPath, MAX_PATH_LENGTH, "%s/../../%s", pg_data, datasetdir);

	if ((dir = opendir(datasetPath)) == NULL)
	{
		fprintf(stderr, "Dataset directory %s not found...", datasetPath);
		return;
	}
	...
	{
		if (content->d_name[0] == '.' && (content->d_name[1] == '\0' || (content->d_name[1] == '.' && content->d_name[2] == '\0')))
			continue;

		/* For now, only consider .csv files */
		if (strstr(content->d_name, ".csv") != NULL)
		{
			fileIndex++;

			snprintf(filePath, MAX_PATH_LENGTH, "%s/%s", datasetPath, content->d_name);

			strncpy(relation, content->d_name, MAX_RELATION_NAME - 1);
			relation[MAX_RELATION_NAME - 1] = '\0';
			getRelationName(relation);
			if (fileIndex != 1)
				fprintf(conffile, "\n", fileIndex, filePath);
			fprintf(conffile, "filename-%d = '%s'\n", fileIndex, filePath);
			fprintf(conffile, "relation-%d = '%s'\n", fileIndex, relation);
			fprintf(conffile, "delimiter-%d = '%c'\n", fileIndex, delimiter);
