...
// Added for configuration file parameters in InvisibleDB
...
static struct config_bool ConfigureNamesBool[] =
	{
		{{"enable_invisible_db", PGC_USERSET, INVISIBLEDB,
		  gettext_noop("InvisibleDB module is enabled."),
		  NULL},
		 &enable_invisible_db,
		 false,
		 NULL,
		 NULL},
		{{"enable_invisible_metadata", PGC_USERSET, INVISIBLEDB,
		  gettext_noop("InvisibleDB: enable PosgresSemiRaw."),
		  NULL},
		 &enable_invisible_metadata,
		 false,
		 NULL,
		 NULL},
		{{"enable_tuple_metapointers", PGC_USERSET, INVISIBLEDB,
		  gettext_noop("InvisibleDB: enable metapointers in the end of each tuple."),
		  NULL},
		 &enable_tuple_metapointers,
		 false,
		 NULL,
		 NULL},
		{{"enable_internal_metapointers", PGC_USERSET, INVISIBLEDB,
		  gettext_noop("InvisibleDB: enable internal metapointers."),
		  NULL},
		 &enable_internal_metapointers,
		 false,
		 NULL,
		 NULL},
		{{"enable_caching", PGC_USERSET, INVISIBLEDB,
		  gettext_noop("InvisibleDB: enable caching."),
		  NULL},
		 &enable_caching,
		 false,
		 NULL,
		 NULL},
		{{"enable_pushdown_select", PGC_USERSET, INVISIBLEDB,
		  gettext_noop("InvisibleDB: enable pushdown selection."),
		  NULL},
		 &enable_pushdown_select,
		 false,
		 NULL,
		 NULL},
...
static struct config_int ConfigureNamesInt[] =
	{
		{{"num_cached_columns", PGC_USERSET, INVISIBLEDB,
		  gettext_noop("Space budget for the storage component"),
		  NULL},
		 &num_cached_columns,
		 -1,
		 -1,
		...
		//		{
		//		{"num_internal_metapointers", PGC_USERSET, INVISIBLEDB,
		//			gettext_noop("Percentage of metapointers per attribute."),
		//			NULL
		//		},
		//		&num_internal_metapointers,
		...
		{{"old_snapshot_threshold", PGC_POSTMASTER, RESOURCES_ASYNCHRONOUS,
		 ...
		 -1,
		 -1,
		 MINS_PER_HOUR *HOURS_PER_DAY * 60,
		 NULL,
		 NULL,
		 NULL},
...
static struct config_string ConfigureNamesString[] =
	{
		{{"conf_file", PGC_USERSET, INVISIBLEDB,
		  gettext_noop("InvisibleDB: Configuration file."),
		  NULL},
	...
	if (!(*conf->check_hook)(newval, extra, source))
	{
		ereport(elevel,
				(errcode(GUC_check_errcode_value),
				 GUC_check_errmsg_string ? errmsg_internal("%s", GUC_check_errmsg_string) : errmsg("invalid value for parameter \"%s\": %d", conf->gen.name, (int)*newval),
				 GUC_check_errdetail_string ? errdetail_internal("%s", GUC_check_errdetail_string) : 0,
				 GUC_check_errhint_string ? errhint("%s", GUC_check_errhint_string) : 0));
					...
					GucSource source, int elevel)
{
	...
	if (!(*conf->check_hook)(newval, extra, source))
	{
		ereport(elevel,
				(errcode(GUC_check_errcode_value),
				 GUC_check_errmsg_string ? errmsg_internal("%s", GUC_check_errmsg_string) : errmsg("invalid value for parameter \"%s\": %d", conf->gen.name, *newval),
				 GUC_check_errdetail_string ? errdetail_internal("%s", GUC_check_errdetail_string) : 0,
				 GUC_check_errhint_string ? errhint("%s", GUC_check_errhint_string) : 0));
		/* Flush any strings created in ErrorContext */
		FlushErrorState();
					 ...
					 GucSource source, int elevel)
{
	...
	if (!(*conf->check_hook)(newval, extra, source))
	{
		ereport(elevel,
				(errcode(GUC_check_errcode_value),
				 GUC_check_errmsg_string ? errmsg_internal("%s", GUC_check_errmsg_string) : errmsg("invalid value for parameter \"%s\": %g", conf->gen.name, *newval),
				 GUC_check_errdetail_string ? errdetail_internal("%s", GUC_check_errdetail_string) : 0,
				 GUC_check_errhint_string ? errhint("%s", GUC_check_errhint_string) : 0));
		/* Flush any strings created in ErrorContext */
		FlushErrorState();
					   ...
					   GucSource source, int elevel)
{
	...
	if (!(*conf->check_hook)(newval, extra, source))
	{
		ereport(elevel,
				(errcode(GUC_check_errcode_value),
				 GUC_check_errmsg_string ? errmsg_internal("%s", GUC_check_errmsg_string) : errmsg("invalid value for parameter \"%s\": \"%s\"", conf->gen.name, *newval ? *newval : ""),
				 GUC_check_errdetail_string ? errdetail_internal("%s", GUC_check_errdetail_string) : 0,
				 GUC_check_errhint_string ? errhint("%s", GUC_check_errhint_string) : 0));
		/* Flush any strings created in ErrorContext */
		FlushErrorState();
					 ...
					 GucSource source, int elevel)
{
	...
	if (!(*conf->check_hook)(newval, extra, source))
	{
		ereport(elevel,
				(errcode(GUC_check_errcode_value),
				 GUC_check_errmsg_string ? errmsg_internal("%s", GUC_check_errmsg_string) : errmsg("invalid value for parameter \"%s\": \"%s\"", conf->gen.name, config_enum_lookup_by_value(conf, *newval)),
				 GUC_check_errdetail_string ? errdetail_internal("%s", GUC_check_errdetail_string) : 0,
				 GUC_check_errhint_string ? errhint("%s", GUC_check_errhint_string) : 0));
		/* Flush any strings created in ErrorContext */
		FlushErrorState();
