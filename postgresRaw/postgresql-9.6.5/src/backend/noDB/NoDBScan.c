...
bool enable_invisible_metadata = false;
...
NoDBScanOperator_t *NoDBScanOperatorInit(ScanState *scanInfo)
{
	...
	char *delimiter;
	bool header;
	Relation temp = (Relation)scanInfo->ss_currentRelation;
	Form_pg_class temp2 = temp->rd_rel;
	char *relation = (char *)temp2->relname.data;
...
void NoDBScanStateReInit(ScanState *scanInfo)
{
	...
	if (cstate->processed >= 0)
	{
		...
		{
			if (!NoDBTryReFillRawBuf(cstate))
			{ // We have to read more tuples from the file -- No Re-scan
...
void NoDBUpdateStrategy(NoDBScanState_t cstate)
{
	...
	if (plan->nRows == 0)
	{
		...
		{
			if (NoDBTryReFillRawBuf(cstate))
			{ // hit_eof -- Strategy to read from file
...
ScanStmtExecutionStatusFree(NoDBScanExecStatusStmt_t status, NoDBScanState_t cstate)
{
	...
	error_context_stack = status.noDBerrcontext.previous;
...
GetScanExecStatusStmt(NoDBScanState_t cstate)
{
	...
	ExecOpenIndices(status.resultRelInfo, false);
	...
	for (attnum = 1; attnum <= status.num_phys_attrs; attnum++)
	{
		...
		InitFunctionCallInfoData(
			cstate->fcinfo[attnum - 1],
			&status.in_functions[attnum - 1],
			3,
			NULL, // Matt: ???
			NULL,
			NULL);
	...
	status.noDBerrcontext.arg = (void *)cstate;
...
traversePlanTree(Plan *plan, PlanState *planstate, Plan *outer_plan, PlannedStmt *topPlan, List *ancestors, ExplainState *es)
{
	...
	if (outerPlan(plan))
	{
		 ...
		 */
		tmpList = traversePlanTree(outerPlan(plan),
...
static void
traverseQual(List *qual, Plan *plan, Plan *outer_plan, PlannedStmt *topPlan, PlanState *planstate, List *ancestors)
{
	List *context;
	Node *node;
	//	char	   *exprstr;
